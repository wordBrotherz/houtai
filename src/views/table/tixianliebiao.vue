<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" label="提現地址">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-link class="m-r-15" title="通過" @click="edit(scope.row)">
            <span style="margin-right: 15px;float: left; color: #216def"
            > {{ scope.row.bill_status == 3 ? '通過' : '' }}  </span>
          </el-link>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { fundingrecharge,withdrawpass } from '@/api/table'
...
export default {
  ...
  methods: {
    ...
    edit(data) {
      let order = prompt('請輸入賬單編號')

      if (order == data.singel_number) {
        this.withdrawpass(data.id)
      } else {
        this.$message.warning('訂單號錯誤')
      }
    },
    onSubmit() {
      this.fetchData()
