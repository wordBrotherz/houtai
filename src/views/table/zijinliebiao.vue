    ...
    <el-form :model="form" label-width="100px">
      ...
      <el-form-item label="狀態" style="float: left;">
        <el-select v-model="form.status" placeholder="狀態選擇">
          ...
          <el-option label="佣金" value="3"/>
          <el-option label="挖矿收益" value="4"/>
          <el-option label="理财利息" value="5"/>
          <el-option label="投资挖矿" value="6"/>
          <el-option label="投资理财" value="7"/>
          <el-option label="挖矿提取" value="8"/>
          <el-option label="理财提取" value="9"/>
        </el-select>
      </el-form-item>
...
<script>
import { fundingshow } from '@/api/table'
...
export default {
  ...
  methods: {
    fetchData() {
      fundingshow(this.form).then(result => {
        if (result[0] == true) {
