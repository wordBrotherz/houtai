        ...
        <div class="liebiao m-t-15" v-for="items in list">
          <div class="lie-2">
            <div class="lie-2-touxiang">
              <img
                src="https://img2.baidu.com/it/u=180286244,263138673&amp;fm=253&amp;fmt=auto&amp;app=138&amp;f=JPEG?w=500&amp;h=500"
              />
            </div>
            <div style="float: right; height: 50px; line-height: 50px; margin-right: 30px;"><b> {{items.username}} </b></div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getassets(value) {
      UserList(value, 200, 1).then(result => {
        if (result[0] == true) {
          this.list = result[1]
        }
      })
