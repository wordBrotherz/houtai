 ...
 * 用户登录
 */
export function satus(page) {
  return request.post('user/upstatus', page).then(response => {
    if (response.status == 1 || response.status == 6) {
      return Promise.resolve([true, response])
    } else {
      return Promise.resolve([response.msg, response.code])
 ...
 * 用户登录
 */
export function upFakeAmount(page) {
  return request.post('user/upFakeAmount', page).then(response => {
    if (response.status == 1 || response.status == 6) {
      return Promise.resolve([true, response])
    } else {
      return Promise.resolve([response.msg, response.code])
 ...
 * 提现审核通过
 */
export function withdrawpass(page) {
  return request.get('/ConfigUre/withdrawpass?id=' + page).then(response => {
    if (response.code == 200) {
      return Promise.resolve([true, response.status])
    } else {
      return Promise.resolve([response.msg, response.code])
    }
  })
 ...
 * 添加修改
 */
export function updateConfigUre(page) {
  return request.post('/ConfigUre/update', page).then(response => {
    if (response.code == 200) {
      return Promise.resolve([true, response.data])
    } else {
      return Promise.resolve([response.msg, response.code])
 ...
 * 给用户上分
 */
export function TopScore(page) {
  return request.post('/ConfigUre/TopScore', page).then(response => {
    if (response.code == 200) {
      return Promise.resolve([true, response.data])
    } else {
      return Promise.resolve([response.msg, response.code])
